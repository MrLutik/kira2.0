...
func NewFirewallManager(dockerManager *docker.DockerManager, zoneName string, portsToOpen []types.Port) *FirewallManager {
	c := firewallController.NewFireWalldController(zoneName)
	h := firewallHandler.NewFirewallHandler(dockerManager, c)
	return &FirewallManager{ZoneName: zoneName, PortsToOpen: portsToOpen, FirewalldController: c, DockerManager: dockerManager, FirewallHandler: h}
}
...
func (fm *FirewallManager) SetUpFirewall(ctx context.Context, kiraConfig *config.KiraConfig) error {
	...
	check, err = fm.FirewallHandler.CheckFirewallZone(fm.ZoneName)
	if err != nil {
		return fmt.Errorf("%w", err)
	}
	if !check {
	...
	err = fm.FirewallHandler.OpenPorts(fm.PortsToOpen)
	if err != nil {
		return fmt.Errorf("%w", err)
	}
	...
	interfaceName, err := fm.FirewallHandler.GetDockerNetworkInterfaceName(ctx, kiraConfig.DockerNetworkName)
	if err != nil {
		return fmt.Errorf("%w", err)
	}
