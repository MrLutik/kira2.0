...
func OpenPort(log *logging.Logger) *cobra.Command {
...
func validateFlags(cmd *cobra.Command, log *logging.Logger) error {
	...
	if utilsOS.CheckIfPortIsValid(portToOpen) {
		return fmt.Errorf("cannot parse port '%s': %w", portToOpen, err)
	}
...
func mainOpenPort(cmd *cobra.Command, log *logging.Logger) {
	var port types.Port
	var err error
	...
	log.Infof("Adding '%s' port with '%s' type to opened list", port.Port, port.Type)
	err = firewallManager.OpenPorts([]types.Port{port})
	if err != nil {
