...
func MasterKeysGen(masterMnemonic []byte, defaultPrefix, defaultPath, masterkeys string) *types.MasterMnemonicSet {
	...
	keys := types.MasterMnemonicSet{}
	if ok {
		fmt.Println("ENTER")
		// VALIDATOR_NODE_MNEMONIC
		keys.ValidatorNodeMnemonic, err = generateFromMasterMnemonic("validator", "node", masterMnemonic)
		if err != nil {
			fmt.Fprintln(os.Stderr, err)
			os.Exit(1)
		}
		...
		// VALIDATOR_NODE_ID
		keys.ValidatorNodeId = generateNodeIdFromMnemonic(string(keys.ValidatorNodeMnemonic))

		//VALIDATOR_ADDR_MNEMONIC
		keys.ValidatorAddrMnemonic, err = generateFromMasterMnemonic("validator", "addr", masterMnemonic)
		if err != nil {
			fmt.Fprintln(os.Stderr, err)
			os.Exit(1)
		}
		...
		keys.ValidatorValMnemonic, err = generateFromMasterMnemonic("validator", "val", masterMnemonic)
		if err != nil {
			fmt.Fprintln(os.Stderr, err)
			os.Exit(1)
		}
		...
		keys.SignerAddrMnemonic, err = generateFromMasterMnemonic("signer", "addr", masterMnemonic)
		if err != nil {
			fmt.Fprintln(os.Stderr, err)
			os.Exit(1)
		}
		...
		if masterkeys != "" {
			...
			ValKeyGen(string(keys.ValidatorNodeMnemonic), defaultPrefix, defaultPath, "",
			...
			ValKeyGen(string(keys.ValidatorValMnemonic), defaultPrefix, defaultPath, fmt.Sprintf("%s/priv_validator_key.json", masterkeys), "", "", false, false, false)

			// mnemonics.env file
			dataToWrite := []byte(fmt.Sprintf("MASTER_MNEMONIC=%s\nVALIDATOR_ADDR_MNEMONIC=%s\nVALIDATOR_NODE_MNEMONIC=%s\nVALIDATOR_NODE_ID=%s\nVALIDATOR_VAL_MNEMONIC=%s\nSIGNER_ADDR_MNEMONIC=%s\n ", masterMnemonic, keys.ValidatorAddrMnemonic, keys.ValidatorNodeMnemonic, keys.ValidatorNodeId, keys.ValidatorValMnemonic, keys.SignerAddrMnemonic))
