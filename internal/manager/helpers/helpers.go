...
func (h *HelperManager) GivePermissionsToAddress(ctx context.Context, permissionToAdd int, address string) error {
	log := logging.Log
	...
	err = h.AwaitNextBlock(ctx, time.Millisecond*time.Duration(h.config.TimeBetweenBlocks))
	if err != nil {
